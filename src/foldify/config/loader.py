...
def get_config_dir() -> Path:
    ...
    config_dir = Path.home() / ".config" / "foldify"
    return config_dir
...
def get_cache_dir() -> Path:
    ...
    cache_dir = Path.home() / ".cache" / "foldify"
...
def ensure_config_dirs() -> None:
    ...
    log_dir = Path.home() / ".local" / "share" / "foldify"
...
def expand_path(path: str) -> Path:
    ...
    handles ~ and env vars in config paths.
    """
...
def load_profile(profile_path: Path) -> Profile:
    ...
    # TODO: add json schema validation for the profile format
    # would help catch errors early instead of at runtime
